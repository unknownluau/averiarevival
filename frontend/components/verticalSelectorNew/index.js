 ...
 * @param {{selected: string; options: {name: string; url: string; disabled?: boolean; el: () => void}[]}} props 
 * @returns 
 */
const VerticalSelector = props => {
  ...
  return <div className={'row mt-4 pe-0 ' + s.row}>
    <div className='col-12 pe-0 me-0'>
      {
        props.options.map(v => {
          return <SelectorOption key={v.name + v.url} onClick={v.el} name={v.name} url={v.url} selected={props.selected === v.name} disabled={v.disabled}/>
        })
      }
    </div>
  </div>
