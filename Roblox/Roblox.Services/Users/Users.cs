...
{
    private static TwoFactorAuth tfa = new TwoFactorAuth("Averia");
    ...
    {
        string ticket = "averia2sv-v2:" + Guid.NewGuid().ToString();
    ...
    {
        string ticket = "averia2svloginv2:" + Guid.NewGuid().ToString() + Guid.NewGuid().ToString();
    ...
    {
        var newName = $"averia_user_{userId}";
        await db.ExecuteAsync(
    ...
    {
        return (await db.QuerySingleOrDefaultAsync<Dto.Total>("SELECT COUNT(*) AS total FROM user_transaction WHERE user_id_one = :user_id AND asset_id = :asset_id AND type = :type AND sub_type = :sub_type AND deleted = true", new
        {
            user_id = userId,
            asset_id = assetId,
            type = PurchaseType.Purchase,
