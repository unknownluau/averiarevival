...
public class GroupsService : ServiceBase, IService
{
    ...
    // Example URL (www is optional): https://www.averia.lol/groups/1/name#!/about
    private static readonly Regex RobloxGroupUrlRegex = new("https:\\/\\/(www\\.)?averia.lol\\/groups\\/[0-9]+\\/[a-zA-Z\\-0-9]+", RegexOptions.IgnoreCase);

    // Example URL (www is optional): https://averia.lol/My/Groups.aspx?gid=4
    private static readonly Regex RobloxGroupUrlRegexOld = new("https:\\/\\/(www\\.)?averia.lol\\/my\\/groups\\.aspx\\?gid=[0-9]+", RegexOptions.IgnoreCase);
    ...
    private static string GetGroupIconUrl(string fileName)
    {
        ...
        foreach (var c in entries)
        {
            results.Add(new FeedEntry
            {
                ...
                group = new FeedCreatorEntry
                {
                    id = c.groupId,
                    name = c.groupName,
                    image = GetGroupIconUrl(c.groupImage),
                },
