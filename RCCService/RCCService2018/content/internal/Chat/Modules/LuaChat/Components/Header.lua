...
function Header.new(appState, dialogType)
	...
	if FFlagLuaChatFlexibleTitleWidth then
		table.insert(self.connections, self.innerTitles:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
			self:SetTitle(self.title) -- Call SetTitle to truncate text according to the new size.
		end))

		table.insert(self.connections, self.innerButtons:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
			self:AdjustTitleHeader()
		end))

		table.insert(self.connections, self.backButton.rbx:GetPropertyChangedSignal("Visible"):Connect(function()
			self:AdjustTitleHeader()
		end))
		self:AdjustTitleHeader()
	end
