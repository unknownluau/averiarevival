...
local Analytics = require(Modules.Shell.Analytics)
...
local MakeSafeAsync = require(Modules.Shell.SafeAsync)

local Game = require(Modules.Shell.Models.Game)
local GameLegacy = require(Modules.Shell.Models.GameLegacy)

local FFlagXboxUseNewGamesEndpoints = settings():GetFFlag("XboxUseNewGamesEndpoints")
--NOTE: Remove these with the FFlagXboxUseNewGamesEndpoints flag
local ReloaderManager = require(Modules.Shell.ReloaderManager)
...
do
	...
	SortsData.DefaultSortId = {
		...
		-- NOTE: To be removed when FFlagXboxUseNewGamesEndpoints is removed
		RecentlyPlayed = -2;
		Favorites = -1;
		TopEarning = 8;
	}
	...
	local updateSortsAync = MakeSafeAsync({
		asyncFunc = function()
			local updateInterval = SortsData:UpdateSorts()
			return updateInterval
		end,
		callback = function(updateInterval)
			wait(updateInterval)
		end,
		userRelated = true,
	})
	local function OnUserAccountChanged()
		...
		spawn(function()
			if startCount == UserChangedCount then
				...
				if FFlagXboxUseNewGamesEndpoints then
					UpdateFuncId = ReloaderManager:addReloaderFunc("SortsData", function() updateSortsAync() end, 0, true)
				else
	...
	--Create a sort
	local function _createSort(id, sortName, httpFunc, userId, gameSetTargetId, useTokens, sortDisplayName, timeFilter)
		...
		sort.Name = sortDisplayName or sortName
		...
		--NOTE: Remove when FFlagXboxUseNewGamesEndpoints is removed and game lists no longer use image ids
		sort.useTokens = useTokens
		if sort.useTokens then
			sort.sortToken = id
			sort.Id = SortsData.DefaultSortId[sortName]
		else
			sort.Id = id
		end
		sort.timeFilter = timeFilter
		...
		if sort.Id == SortsData.DefaultSortId.TopRated and sort.GameSetTargetId == 0 then
			sort.timeFilter = sort.timeFilter or 2
		elseif sort.Id == SortsData.DefaultSortId.Favorites and sort.GameSetTargetId == 0 then
			sort.eventString = EventHub.Notifications["FavoriteToggle"]
			sort.objectIDString = tostring(sort.Id)
		...
		elseif sort.Id == SortsData.DefaultSortId.RecentlyPlayed and sort.GameSetTargetId == 0 then
		...
		function sort:UpdateCachedGamesData(data, pagingIndex, scrollLeft)
			...
			for i = indexFrom, indexTo, step do
				...
				if FFlagXboxUseNewGamesEndpoints then
					local game;
					if self.useTokens then
						game = Game.fromJsonData(data[dataIndex])
					else
						game = GameLegacy.fromJsonData(data[dataIndex])
					end
					...
					placeId = data[dataIndex]["PlaceID"]
...
--				Utility.DebugLog("SortsData: has cached data", pageIndex, hasPrevPage)
		...
		function sort:GetSortAsync(startIndex, pageSize)
			...
			if not hasCached then
				local data;
				if self.useTokens then
					data = self.httpFunc(startIndex, pageSize, self.sortToken, self.timeFilter)
				else
					data = self.httpFunc(startIndex, pageSize, self.userId or self.Id, self.timeFilter)
				end
				local listData;
				if data then
					listData = self.useTokens and data.games or data
					for i = 1, #listData do
						local placeId;
						if FFlagXboxUseNewGamesEndpoints then
							local game;
							if self.useTokens then
								game = Game.fromJsonData(listData[i])
							else
								game = GameLegacy.fromJsonData(listData[i])
							end
							placeId = game.placeId
							FlushGameData(startIndex + i)
							GameData:UpdateGameData(game)
						else
							local creatorName = listData[i]["CreatorName"]
							local iconId = listData[i]["ImageId"]
							local name = listData[i]["Name"]
							placeId = listData[i]["PlaceID"]
							local voteData = {
								UpVotes = listData[i]["TotalUpVotes"];
								DownVotes = listData[i]["TotalDownVotes"];
							}
							local creatorId = listData[i]["CreatorID"]
...
--					Utility.DebugLog("sortsData:InitSortAsync: pageIndex", i, "hasPage", page and #page)
	...
	--NOTE: Remove when FFlagXboxUseNewGamesEndpoints is removed and game list no longer use image ids
	local function createSort(sortId, sortName, httpFunc, userId, gameSetTargetId)
		return _createSort(sortId, sortName, httpFunc, userId, gameSetTargetId, false)
	end

	--NOTE: Change name to createSort when cleaning up FFlagXboxUseNewGamesEndpoints and all game lists use tokens
	local function createSortWithToken(sortKoken, sortName, httpFunc, userId, gameSetTargetId, sortDisplayName, timeFilter)
		return _createSort(sortKoken, sortName, httpFunc, userId, gameSetTargetId, true, sortDisplayName, timeFilter)
	end
	...
	local function GetSortFromSortId(SortId, GameSetTargetId)
		local sort = nil
		if VisibleSortsDataCached and VisibleSortsDataCached.Data and VisibleSortsId_IndexMap and
			VisibleSortsId_IndexMap[SortId] and VisibleSortsId_IndexMap[SortId][GameSetTargetId or 0] then
			sort = VisibleSortsDataCached.Data[VisibleSortsId_IndexMap[SortId][GameSetTargetId or 0]]
		end
		return sort or createBaseSort()
	end

	-- Finds the time filter token
	local function GetTimeFilter(timeFilterName, timeFilters)
		for _,timeFilter in ipairs(timeFilters) do
			if timeFilter["name"] == timeFilterName then
				return timeFilter["token"]
			end
		end
		return nil
	end
	...
	local function GetSortsAsync(userId)
		...
		local tokenExpiry;
		if FFlagXboxUseNewGamesEndpoints then
			local result = Http.GetGameSortsAsync2()
			if result then
				local timeFilters = result["timeFilters"]
				for i = 1, #result.sorts do
					--ignore sort that GameSetTargetId ~= 0
					local gameSetTargetId = result.sorts[i]["GameSetTargetId"] or 0
					if gameSetTargetId == 0 then
						local sortToken = result.sorts[i]["token"]
						local sortName = result.sorts[i]["name"]
						local sortDisplayName = result.sorts[i]["displayName"]
						-- Get the id from this sort name that matches the id used by the old endpoint
						local sortId = SortsData.DefaultSortId[sortName]
						local tokenExpiryInSeconds = result.sorts[i]["tokenExpiryInSeconds"]
						if tokenExpiry == nil then
							tokenExpiry = tokenExpiryInSeconds
						else
							tokenExpiry = math.min(tokenExpiry,tokenExpiryInSeconds)
						end
						--Check if this is a sort that we want to include
						if sortId then
							local timeFilter;
							if sortName == "TopRated" then
								timeFilter = GetTimeFilter("PastWeek", timeFilters)
							end
							local sort = createSortWithToken(sortToken, sortName, Http.GetSortAsync2, nil, nil, sortDisplayName, timeFilter)
	...
	--					Utility.DebugLog(i, "/", #result, ".SortsData.GetSortsAsync", sortId, ", ", gameSetTargetId, ", ", sortName)
		...
		if FFlagXboxUseNewGamesEndpoints == true then
			--TODO: Need new en dpoint.  Need to include the my games sort into the games sorts. However, this end point still use id for thumbnails.
		...
		if FFlagXboxUseNewGamesEndpoints then
			local timeDiff = tick() - time
			return Sorts, tokenExpiry - timeDiff
		else
			return Sorts
		end
	end
	...
	function SortsData:UpdateSorts()
		...
		local startCount = UserChangedCount
		local newSorts, tokenExpiry = GetSortsAsync(userId)
