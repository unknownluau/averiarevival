	...
	for i = 1, #images do
		if imageIds[i] then
			if FFlagXboxUseNewGamesEndpoints then
				BatchThumbnailLoader:CreateThumbnail(images[i], imageIds[i], BatchThumbnailLoader.Sizes.Medium, true, false)
			else
		...
		if placeIds then
			for i = 1, #images do
				...
				if placeId then
					...
					if gameData then
						if images[i] then
							if FFlagXboxUseNewGamesEndpoints and gameData.ImageToken then
								BatchThumbnailLoader:CreateThumbnail(images[i], gameData.ImageToken, BatchThumbnailLoader.Sizes.Medium, true, false)
							else
