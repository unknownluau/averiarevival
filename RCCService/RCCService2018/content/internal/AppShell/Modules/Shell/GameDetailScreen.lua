...
--[[ Scrolling Content Items ]]--
	...
	local function loadGameIcon(data)
		if FFlagXboxUseNewGamesEndpoints and data.ImageToken then
			BatchThumbnailLoader:CreateThumbnail(GameIconImage, data.ImageToken, BatchThumbnailLoader.Sizes.Medium, true, true, { ZIndex = GameIconImage.ZIndex })
		else
			local gameIconLoader = ThumbnailLoader:Create(GameIconImage, data.IconId,
	...
	local DescriptionListLayout = Utility.Create"UIListLayout"
	{
...
--[[ Badges ]]--
		...
		if #placeIds > 0 then
			for i = 1, #RelatedGameImages do
				...
				if placeIds[i] then
					...
					if FFlagXboxUseNewGamesEndpoints and this.ImageToken then
						BatchThumbnailLoader:CreateThumbnail(image, this.ImageToken, BatchThumbnailLoader.Sizes.Medium, true, true, { ZIndex = image.ZIndex } )
					else
	...
	local function setDescriptionDisplay(overrideAvatar, isExperimental)
		-- TODO: CLIXBOX-1987 - need running total w/o quantum gui
		local runningHeight = 0

		local function createTextLabel(text, name, layoutOrder)
			local width = RatingDescriptionContainer.Size.X.Offset
			local textSize = GlobalSettings.TextSizes.SubHeader
			local font = GlobalSettings.BoldFont

			local size = TextService:GetTextSize(text, textSize, font, Vector2.new(0, 0))

			local lineCount = math.ceil(size.x/width)
			local height = textSize * lineCount

			runningHeight = runningHeight + height + DescriptionListLayout.Padding.Offset
		...
		if overrideAvatar then
			local text = Strings:LocalizedString("CustomAvatarPhrase")
			createTextLabel(text, "OverrideAvatarText", 1)
		end

		if isExperimental then
			local text = Strings:LocalizedString("ExperimentalGamePhrase")
			createTextLabel(text, "ExperiementalText", 2)
		end

		local remainingSize = DescriptionContainer.Size.Y.Offset - runningHeight

		DescriptionScrollingTextBox:SetSize(UDim2.new(1, 0, 0, remainingSize))
		DescriptionScrollingTextBox:SetLayoutOrder(3)
	end
	...
	do
		...
		local function loadDescriptionDataAsync()
			...
			setDescriptionDisplay(overridesDefaultAvatar, data.IsExperimental)
