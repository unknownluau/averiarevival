...
local XboxUseNewBundleEndpoints = settings():GetFFlag("XboxUseNewBundleEndpoints")
...
local Bundle = require(ShellModules.Models.Bundle)
local PackageItem = require(ShellModules.PackageItem)
...
do
	...
	local function GetAvailableXboxCatalogPackagesAsync()
		...
		repeat
			...
			Utility.ExponentialRepeat(
				...
				function() result = Http.GetXboxProductsAsync(index, count) end, 2)
	...
	local function getXboxExclusiveBundlesAsync()
		local result;
		local function getBundlesPredicate()
			return result == nil
		end

		local function doGetBundles()
			result = Http.GetXboxExclusiveItemsAsync()
		end

		Utility.ExponentialRepeat(getBundlesPredicate, doGetBundles, 2)

		local bundles = {}
		if result and result.data then
			-- get details and partIds for each bundle item and attach additional data
			for _,item in pairs(result.data) do
				local detailsResult;
				local function getBundleDetailsPredicate()
					return detailsResult == nil
				end

				local function doGetBundleDetails()
					detailsResult = Http.GetBundleDetailsAsync(item.id)
				end

				Utility.ExponentialRepeat(getBundleDetailsPredicate, doGetBundleDetails, 2)

				if detailsResult then
					if detailsResult.product then
						item.productId = detailsResult.product.id
					end

					if detailsResult.creator then
						item.creatorId = detailsResult.creator.id
					end
				end

				local partsResult = Http.GetPackageAssetsAsync(item.id)
				if partsResult and partsResult.assetIds then
					item.partIds = partsResult.assetIds
					table.sort(item.partIds)
				end

				table.insert(bundles, Bundle.fromData(item))
			end
	...
	local function getOwnedPackagesAsync()
		local result;
		local function getOwnedPackagesPredicate()
			return result == nil
		end

		local function doGetOwnedPackages()
			result = Http.GetUserOwnedPackagesAsync(XboxAppState.store:getState().RobloxUser.rbxuid)
		end

		Utility.ExponentialRepeat(getOwnedPackagesPredicate, doGetOwnedPackages, 2)

		local packages = {}

		if result and result.IsValid and result.Data and result.Data.Items then
			for _,item in pairs(result.Data.Items) do
				local data = {}

				if item.Item then
					data.id = item.Item.AssetId
				...
				if item.Product then
					data.priceInRobux = item.Product.PriceInRobux or 0
				end

				local productInfoResult = MarketplaceService:GetProductInfo(data.id)
				if productInfoResult then
					data.description = productInfoResult.Description
					data.productId = productInfoResult.ProductId
				end

				local partsResult;
				local function getPackageAssetsPredicate()
					return partsResult == nil
				end

				local function doGetPackageAseets()
					partsResult = Http.GetPackageAssetsAsync(data.id)
				end

				Utility.ExponentialRepeat(getPackageAssetsPredicate, doGetPackageAseets, 2)

				if partsResult and partsResult.assetIds then
					data.partIds = partsResult.assetIds
					table.sort(data.partIds)
				end

				table.insert(packages, Bundle.fromData(data))
			end
		end
	...
	local function getCatalogPackagesAsync()
		if XboxUseNewBundleEndpoints then
			local xboxBundles = getXboxExclusiveBundlesAsync()
			local myPackages = getOwnedPackagesAsync()
			...
			if xboxBundles then
				for _,data in pairs(xboxBundles) do
					local packageItem = PackageItem.new(data)
					local assetId = packageItem:GetAssetId()
					local owned = myPackages[assetId] ~= nil
					result.OwnedInfo[assetId] = owned
					packageItem:SetOwned(owned)
					table.insert(result.Packages, packageItem)
				end
			end

			if myPackages then
				for _, data in ipairs(myPackages) do
					local assetId = data.assetId
					if result.OwnedInfo[assetId] == nil then
						local packageItem = PackageItem.new(data)
						result.OwnedInfo[assetId] = true
						packageItem:SetOwned(true)
						table.insert(result.Packages, packageItem)
					end
				end
			end
			...
			if packages then
				...
				if XboxUseNewBundleEndpoints then
					for _, package in pairs(packages) do
						if CompareAssetIdArrays(package:GetPartIds(), wornAssetIds) then
				...
				if not VisiblePackageDataCache then
					...
					EventHub:addEventListener(EventHub.Notifications["CharacterEquipped"], "VisiblePackageDataCache",
						function(newAssets, skipProfileImageUpdate)
							...
							if XboxUseNewBundleEndpoints then
								for _, package in pairs(packages) do
									if CompareAssetIdArrays(package:GetPartIds(), assetsArray) then
										newWearingAssetId = tonumber(package:GetAssetId())
										break
									end
								end
	...
	function PackageData:GetCachedData()
		...
		if VisiblePackageDataCache then
			...
			for _, package in pairs(VisiblePackageDataCache.Packages.Data) do
				...
				if VisiblePackageDataCache.OwnedInfo[assetId] and VisiblePackageDataCache.OwnedInfo[assetId].Data then
