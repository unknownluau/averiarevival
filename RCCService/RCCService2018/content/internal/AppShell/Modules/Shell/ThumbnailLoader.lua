...
ThumbnailLoader.AssetType = {
	Icon = { IsFinal = Http.GetAssetThumbnailFinalAsync;
			 SetImageUrl = AssetGameBaseUrl..'Thumbs/Asset.ashx?width=%d&height=%d&assetId=%d&ignorePlaceMediaItems=true'; };
	SquareIcon = { IsFinal = Http.GetAssetThumbnailFinalAsync;
			 SetImageUrl = AssetGameBaseUrl..'Thumbs/GameIcon.ashx?width=%d&height=%d&assetId=%d&ignorePlaceMediaItems=true'; };
	Avatar = { IsFinal = Http.GetAssetAvatarFinalAsync;
			 SetImageUrl = AssetGameBaseUrl..'Thumbs/Avatar.ashx?width=%d&height=%d&userId=%d&ignorePlaceMediaItems=true'; };
	Outfit = { IsFinal = Http.GetOutfitThumbnailFinalAsync;
			 SetImageUrl = BaseUrl..'outfit-thumbnail/image?width=%d&height=%d&userOutfitId=%d'; };
}
...
function ThumbnailLoader:Create(imageObject, assetId, size, assetType, cachebust)
	...
	local uri = string.format(assetType.SetImageUrl, size.x, size.y, assetId or -1)
