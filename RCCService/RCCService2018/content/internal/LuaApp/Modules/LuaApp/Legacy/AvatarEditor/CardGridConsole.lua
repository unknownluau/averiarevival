...
local XboxAvatarEditorOutfitThumbnailFix = Flags:GetFlag("XboxAvatarEditorOutfitThumbnailFix")
...
local function createCardGrid(scrollingFrame, getAssetList, characterManager)
	...
	local function makeNewAssetCard(cardName)
	...
	function this:makeAssetCard(i, cardName, image, clickFunction, isEquipped, unAvailable, YOffset)
		local card = popRecycledAssetCard() or makeNewAssetCard(cardName)
		...
		--TODO: clean up when remove XboxAvatarEditorOutfitThumbnailFix
	...
	function this:makeOutfitCard(i, assetId, thumbnailSize, thumbnailType, clickFunction)
		local cardName = string.format("%s#%s#%s", tostring(assetId), tostring(thumbnailType), tostring(thumbnailSize))
		local card = popRecycledAssetCard() or makeNewAssetCard(cardName)
		card.AssetItemImage.Image = ""
		...
		card.Position = UDim2.new(
			...
			(column-1) * (BUTTONS_SIZE + GRID_PADDING),
			0,
			(row - 1) * (BUTTONS_SIZE + GRID_PADDING)
		)
		...
		local myVersionKey = (assetCardVersionKeys[card] or 0) + 1
		assetCardVersionKeys[card] = myVersionKey
		local dummyImage = Utilities.create'ImageLabel'({})
		local thumbLoader = ThumbnailLoader:Create(dummyImage, assetId, thumbnailSize, thumbnailType, true)
		Utilities.fastSpawn(function()
			local isFinalSuccess = thumbLoader:LoadAsync(false, false)
			if isFinalSuccess and myVersionKey == assetCardVersionKeys[card] then
				card.AssetItemImage.Image = dummyImage.Image
			end
		end)

		card.AssetButtonBackground.Image = LayoutInfo.ItemAvailableBackgroundImage
		card.AssetBorderMask.Image = LayoutInfo.ItemMaskImage
		return card
	end
	...
	local function renderAssetCardByIndex(i)
		...
		if currentPage.specialPageType == 'Outfits' or currentPage.specialPageType == 'Recent Outfits' then
			...
			local card;
			if XboxAvatarEditorOutfitThumbnailFix then
				card = this:makeOutfitCard(i, assetId, ThumbnailLoader.Sizes.Small, ThumbnailLoader.AssetType.Outfit, clickFunction)
			else
				card = this:makeAssetCard(i, cardName, cardImage, clickFunction)
			end
