...
return function(networkImpl, sortCategory, targetSort, optionalSettings)
	...
	return function(store)
		...
		if not targetSort then
			for _, sortName in ipairs(state.GameSortGroups[sortCategory].sorts) do
				...
				local promise = store:dispatch(ApiFetchGamesInSort(networkImpl, sort, nil, optionalSettings))
				table.insert(fetchPromises, promise)
			end
		else
			if state.GameSorts[targetSort] then
				local promise = store:dispatch(ApiFetchGamesInSort(networkImpl, state.GameSorts[targetSort], nil, optionalSettings))
				table.insert(fetchPromises, promise)
			else
				Promise.reject()
			end
