...
local luaAppRefreshScrollingFrameRefactor = settings():GetFFlag('LuaAppRefreshScrollingFrameRefactor')
...
function RefreshScrollingFrame:init()
	...
	-- TODO: remove with luaAppRefreshScrollingFrameRefactor
	self.shouldRefreshOnScroll = false
	self._isMounted = false

	if luaAppRefreshScrollingFrameRefactor then
	...
	self.inputEndedCallback = function(input)
		...
		if self.state.offset < -REFRESH_THRESHOLD and not self.state.activated then
			...
			if not luaAppRefreshScrollingFrameRefactor then
...
function RefreshScrollingFrame:render()
	...
	local shouldShowSpinner = true

	if luaAppRefreshScrollingFrameRefactor then
		shouldShowSpinner = activated or tween or self._isUserInteracting
	end
	...
	local bottomBarButtonPressedEventConnection

	if luaAppRefreshScrollingFrameRefactor then
		bottomBarButtonPressedEventConnection = Roact.createElement(ExternalEventConnection, {
		...
		bottomBarButtonPressedEventConnection = refreshOnNavBar and Roact.createElement(ExternalEventConnection, {
			event = notificationService.RobloxEventReceived,
	...
	return Roact.createElement("Frame", {
		...
		bottomBarButtonPressed = bottomBarButtonPressedEventConnection,
	})
end
