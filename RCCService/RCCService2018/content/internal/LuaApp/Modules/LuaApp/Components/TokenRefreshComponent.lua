...
function TokenRefreshComponent:init()
	self.steppedCallback = function()
		...
		if currentTime > nextTokenRefreshTime then
			self.props.refresh(self.props.networking, sortToRefresh):catch(function()
...
TokenRefreshComponent = RoactRodux.UNSTABLE_connect2(
	...
	function(dispatch)
		return {
			refresh = function(networking, sortToRefresh)
				return dispatch(ApiFetchSortTokens(networking, sortToRefresh))
			end,
		}
	end
