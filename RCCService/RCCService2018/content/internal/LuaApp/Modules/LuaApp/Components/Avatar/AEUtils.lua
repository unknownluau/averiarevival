...
function AEUtils.generateNewId()
    newId = newId + 1
    return newId
end

function AEUtils.getCurrentPage(categoryIndex, tabsInfo)
...
function AEUtils.getThumbnail(isOutfit, assetId)
    local result
    if isOutfit then
        result = AEWebApi.GetOutfitImage(assetId)
    else
        result = AEWebApi.GetAssetImage(assetId)
    end

    return result
end

function AEUtils.isAssetEquipped(assetId, assetTypeId, equippedAssets)
...
function AEUtils.getEquippedAssetIds(equippedAssets)
    local assetIds = {}
    for _, assetTypeId in pairs(AEConstants.AssetTypes) do
...
function AEUtils.delta_CIEDE2000(c1, c2)
