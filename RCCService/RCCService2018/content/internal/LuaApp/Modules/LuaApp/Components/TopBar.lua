...
local searchPageDispatchesInitialSearch = settings():GetFFlag('LuaSearchPageDispatchesInitialSearch')
...
function TopBar:updateTopBarHeight()
	if self.props.setTopBarHeight then
		local newTopBarHeight = self.state.statusBarHeight + NAV_BAR_SIZE + self.state.siteMessageBannerHeight
...
function TopBar:init()
	...
	self.confirmSearchCallback = function(keyword)
		...
		if searchPageDispatchesInitialSearch then
...
function TopBar:render()
	local statusBarHeight = self.state.statusBarHeight
...
TopBar = RoactRodux.UNSTABLE_connect2(
	...
	function(dispatch)
		return {
			...
			-- TODO: remove with flag LuaSearchPageDispatchesInitialSearch
