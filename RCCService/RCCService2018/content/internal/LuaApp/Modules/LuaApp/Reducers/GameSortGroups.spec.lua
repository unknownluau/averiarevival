return function()
	...
	describe("SetGameSortsInGroup", function()
		...
		it("should set the game sorts in group", function()
			...
			local expectedAddedSorts = TableUtilities.FieldCount(testGameSorts)

			local defaultState = GameSortGroups(nil, {})

			-- check that there are no sorts in the expected group to begin with
			local totalSorts = TableUtilities.FieldCount(defaultState[expectedModifiedGroup].sorts)
			...
			totalSorts = TableUtilities.FieldCount(modifiedState[expectedModifiedGroup].sorts)
