		...
		<div class="row">
					...
					<div class="col-12 col-lg-4">
						<div class="card bg-dark">
							...
							<div class="card-body">
								...
								<img class="avatar-thumb" alt={`${info.data.username}'s Avatar`}
									 src={info.data.thumbnail_url} />
								<p>
									<a href={`/users/${info.data.id}/profile`}>
										<LinkIcon />
										View Averia Profile</a>
								</p>
							</div>
						</div>
					</div>
						...
						<div class="card bg-dark text-light">
							...
							{#if (!info.data.is_moderator && !info.data.is_admin && !info.data.joinApp && !info.data.invite)}
								<div class="card card-body card-header bg-warning">
									<p class="mb-0">User does not have an invite or join app. They cannot access the
										website.</p>
								</div>
							{/if}
							<div class="card-body">
								<p class="mb-0">Joined {moment(info.data.created_at).fromNow()}
									({moment(info.data.created_at).format("MMM DD YYYY, h:mm A")})</p>
								<p class="mb-0">Last Online {moment(info.data.online_at).fromNow()}
									({moment(info.data.online_at).format("MMM DD YYYY, h:mm A")})</p>
								<p class="mb-2">
									<span
										class="badge bg-success">{typeof info.data.balance_robux === "number" ? info.data.balance_robux.toLocaleString() : "0"}
										Robux</span>
									<span
										class="badge bg-warning">{typeof info.data.balance_tickets === "number" ? info.data.balance_tickets.toLocaleString() : "0"}
										Tix</span>
								</p>
								<div class="row">
									<div class="col-6">
											...
											<tr>
												...
												<td>
													...
													{:else if info.data.invite}
														<span>Invited by <a
															href={`/admin/manage-user/${info.data.invite.authorId}`}>#{info.data.invite.authorId}</a></span>
													{:else}
														N/A
													{/if}
												</td>
								...
								{:else}
									...
									<p class="mb-0"><span class="fw-bold">Reason</span>: <span
										class="monospace">{info.data["ban_reason"] || "No reason specified"}</span></p>
									<p class="mb-0"><span class="fw-bold">Internal Reason</span>: <span
										class="monospace">{info.data["ban_reason_internal"] || "No internal reason specified"}</span>
									</p>
									<p class="mb-0"><span class="fw-bold">Author</span>: <a
										href={`/users/${info.data["ban_author_user_id"]}/profile`}>{info.data["ban_author_username"] || "No author"}</a>
									</p>
									<p class="mb-0"><span
										class="fw-bold">Created</span>: {moment(info.data["ban_created_at"]).format("MMM DD YYYY, h:mm A")}
									</p>
									<p class="mb-0"><span
										class="fw-bold">Expiration</span>: {info.data["ban_expired_at"] ? moment(info.data["ban_expired_at"]).format("MMM DD YYYY, h:mm A") : 'Never'}
									</p>
								{/if}
							</div>
						</div>
			...
			<div class="col-12 col-md-6 col-lg-3 actions">
				...
				{#if rank.hasPermission("GiveUserRobux")}
					<a use:link class="btn-outline-primary btn w-100" href={`/admin/manage-robux-user/${userId}`}>
						<DollarSignIcon />
						Manage Currency</a>
				{/if}
				{#if rank.hasPermission("GetUserTransactions")}
					<a use:link class="btn-outline-primary btn w-100" href={`/admin/user-transactions/${userId}`}>
						<DollarSignIcon />
						Review Transactions</a>
					<a use:link class="btn-outline-primary btn w-100" href={`/admin/user-trades/${userId}`}>
						<DatabaseIcon />
						Review Trades</a>
				{/if}
				...
				<a use:link class="btn-outline-primary btn w-100" href={`/admin/message-user/${userId}`}>
					<MailIcon />
					Send Message</a>

				{#if rank.hasPermission("GiveUserBadge") || rank.hasPermission("DeleteUserBadge")}
					<a use:link class="btn-outline-primary btn w-100" href={`/admin/manage-badges-user/${userId}`}>
						<AwardIcon />
						Manage Badges</a>
				{/if}

				<Permission p="DeleteUsername">
					<a use:link class="btn-outline-primary btn w-100" href={`/admin/manage-usernames/${userId}`}>
						<MailIcon />
						Manage Usernames</a>
				</Permission>
